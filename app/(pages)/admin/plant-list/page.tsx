...
const PLANT_STATUS_COLORS: Record<string, string> = {
  ...
  DEPLOYED: "bg-blue-100 text-blue-700 dark:bg-blue-950/50 dark:text-blue-300",
  RETURNED: "bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400",
  DAMAGED: "bg-red-100 text-red-700 dark:bg-red-950/50 dark:text-red-300",
  LOST: "bg-orange-100 text-orange-700 dark:bg-orange-950/50 dark:text-orange-300",
};
...
export default function PlantListPage() {
  ...
  function openCreate() {
    setEditing(null);
    setForm({ name: "", sku: "", description: "", categoryId: "", supplierId: "", thumbnailUrl: "", isReturnable: true, sizesRaw: "", colorsRaw: "" });
    setDialogOpen(true);
  }
  ...
  const columns: ColumnDef<PlantItem>[] = [
    {
      ...
      cell: ({ row }) =>
        row.original.thumbnailUrl ? (
          <img src={row.original.thumbnailUrl} alt={row.original.name} className="h-10 w-10 rounded object-cover" />
        ) : (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!sizes.length) return <span className="text-muted-foreground text-xs">—</span>;
        return (
          <div className="flex flex-wrap gap-1">
            {sizes.map((s) => (
              <span key={s} className="rounded border border-border bg-muted/60 px-1.5 py-0.5 text-xs font-medium">{s}</span>
            ))}
          </div>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (!colors.length) return <span className="text-muted-foreground text-xs">—</span>;
        return (
          <div className="flex flex-wrap gap-1">
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return count > 0 ? (
          <Badge variant="secondary">{count} site{count !== 1 ? "s" : ""}</Badge>
        ) : (
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-1">
          ...
          <Button
            ...
            title={(row.original._count?.plantAssignments ?? 0) > 0 ? "Cannot delete: currently deployed" : "Delete"}
          >
    ...
    <div className="mx-auto w-full max-w-7xl space-y-4">
      ...
      {!loading && filtered.length === 0 ? (
        <div className="border border-dashed border-zinc-300 dark:border-zinc-700/50 bg-white/50 dark:bg-card/30 p-12 text-center rounded-lg">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Add step ladders, compressors, generators, safety harnesses and other equipment.</p>
        </div>
      ) : (
        <div className="border bg-card rounded-lg overflow-hidden">
          ...
          <div className="flex items-center justify-between border-t px-4 py-3 bg-muted/60">
            <div className="text-muted-foreground hidden text-sm lg:flex">
              Showing {filtered.length === 0 ? 0 : table.getState().pagination.pageIndex * table.getState().pagination.pageSize + 1}{" "}
              to {Math.min((table.getState().pagination.pageIndex + 1) * table.getState().pagination.pageSize, filtered.length)}{" "}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editing ? "Edit Plant Item" : "Add Plant Item"}</DialogTitle>
            <DialogDescription>
              {editing ? "Update details for this plant/equipment item." : "Add equipment such as step ladders, compressors, generators, harnesses, etc."}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
              ...
              {form.thumbnailUrl ? (
                <div className="rounded border-2 border-dashed border-green-500 bg-green-50/30 dark:bg-green-950/20 p-3">
                  <div className="flex items-end gap-3">
                    ...
                    <div className="flex-1"><p className="text-sm font-medium text-green-700 dark:text-green-300">✓ Image uploaded</p></div>
                    <button type="button" onClick={() => setForm({ ...form, thumbnailUrl: "" })}
                      className="rounded bg-white p-1.5 hover:bg-red-50 text-red-600 dark:bg-zinc-800 transition">
                      <X className="h-4 w-4" />
                    </button>
                  </div>
                </div>
                ...
                >
                  <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleThumbnailUpload(f); e.target.value = ""; }} disabled={uploading} />
                  <div className="flex flex-col items-center gap-1.5">
                    <div className="rounded-full bg-blue-100 p-2 dark:bg-blue-950/50"><Upload className="h-4 w-4 text-blue-600" /></div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Step Ladder 3m" />
              </div>
              <div className="space-y-1">
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium">
                Sizes <span className="text-muted-foreground font-normal">(comma-separated)</span>
              </label>
              <Input
                value={form.sizesRaw}
                onChange={(e) => setForm({ ...form, sizesRaw: e.target.value })}
                placeholder='e.g. 3m, 5m, 8m  or  Small, Medium, Large  or  S, M, L, XL'
              />
              {form.sizesRaw && (
                <div className="flex flex-wrap gap-1 pt-1">
                  {parseTags(form.sizesRaw).map((s) => (
                    <span key={s} className="rounded border border-border bg-muted/60 px-2 py-0.5 text-xs font-medium">{s}</span>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium">
                Colors <span className="text-muted-foreground font-normal">(comma-separated, optional)</span>
              </label>
              <Input
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} disabled={submitting}>{submitting ? "Saving…" : editing ? "Update" : "Add Item"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <ConfirmationDialog
        ...
        onConfirm={handleDelete} confirmText="Remove" variant="destructive"
      />
    </div>
  );
}
