...
/**
 * GET /api/app/admin/product-categories
 */
export async function GET(req: Request) {
  try {
    ...
    const categories = await prisma.productCategory.findMany({
      orderBy: { name: "asc" },
      include: { _count: { select: { procurementProducts: true } } },
    });

    return NextResponse.json({ ok: true, data: categories }, { headers: CORS });
  } catch (e: any) {
...
/**
 * POST /api/app/admin/product-categories
 * Body: { name }
 */
