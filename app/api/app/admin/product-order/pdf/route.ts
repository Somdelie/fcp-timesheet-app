...
export async function POST(req: NextRequest) {
  ...
  let data: ProductOrderVoucherData;
  try {
    data = await req.json();
  } catch {
    return NextResponse.json(
  ...
  const pdfDoc = React.createElement(ProductOrderVoucherDocument, {
    data,
  ...
  const safeRecipient = data.recipientName
    .replace(/[^a-zA-Z0-9 _-]/g, "")
    .replace(/\s+/g, "_")
    .substring(0, 40);
  const safeOrder = data.orderNumber.replace(/[^a-zA-Z0-9-]/g, "");
  const filename = `Product_Order_${safeRecipient}_${safeOrder}.pdf`;
