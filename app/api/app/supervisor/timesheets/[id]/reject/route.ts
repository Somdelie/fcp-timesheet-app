...
export async function POST(
  ...
  if (auth.role !== "SUPERVISOR")
    return NextResponse.json(
      { error: "Forbidden - only supervisors can reject timesheets" },
  ...
  // Require siteId for per-site rejection
  if (!parsed.siteId) {
    return NextResponse.json(
      {
        error:
          "siteId is required for rejection. Use format: YYYY-MM-DD_YYYY-MM-DD_FOREMANID_SITEID",
      },
      { status: 400, headers: CORS },
    );
  }
  ...
  // Admins can reject any timesheet; supervisors need site access check
  if (auth.role === "SUPERVISOR") {
  ...
  if (ts.status !== "SUBMITTED") {
    return NextResponse.json(
      {
        error: `Only SUBMITTED timesheets can be rejected (current: ${ts.status}).`,
      },
      { status: 400, headers: CORS },
    );
  }

  // Optional: accept rejectionReason from body later
