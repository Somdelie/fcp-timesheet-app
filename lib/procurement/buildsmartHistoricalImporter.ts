...
export async function importBuildSmartRows(
  ...
  for (const row of rows) {
    ...
    // Layer 1: dedup by invoice ref when available
    if (row.externalRef && row.ledgerCode) {
      ...
      if (existing) {
        results.push({ row, status: "DUPLICATE_IMPORTED", reason: "Already imported", category, siteId });
        continue;
      }
    }

    // Layer 2: exact-match fallback — prevents duplicates even when ref parsing changes
    {
      ...
      if (exactMatch) {
        results.push({ row, status: "DUPLICATE_IMPORTED", reason: "Already imported (exact match)", category, siteId });
        continue;
      }
    }

    if (row.transactionDate >= APP_ADOPTION_DATE) {
      const overlaps = await appRecordExists(siteId, category, row.transactionDate, amount);
      if (overlaps) {
        results.push({
          row,
