...
function extractForemanName(detail: unknown): string {
  ...
  const d =
    (anyDetail?.timesheet as Record<string, unknown>) ??
    (detail as Record<string, unknown>);

  const foreman = d?.foreman as Record<string, unknown> | undefined;
  const user = foreman?.user as Record<string, unknown> | undefined;

  const candidates = [
    foreman?.name,
    user?.name,
    d?.foremanName,
    user?.displayName, // if you ever add it
  ];
...
export function normalizeTimesheetToGrid(detail: unknown): TimesheetGridModel {
  ...
  const d = ((raw?.timesheet as Record<string, unknown>) ?? raw) as Record<
    string,
    unknown
  >;
  ...
  const columns: TimesheetGridModel["columns"] = (
    ...
    const iso = safeStr(col.iso);
    const fallbackDay = safeStr(col.day);
  ...
  const rows: TimesheetGridModel["rows"] = ((d?.rows ?? []) as unknown[]).map(
    (r: unknown) => {
      const row = r as {
        id?: unknown;
        employeeId?: unknown;
